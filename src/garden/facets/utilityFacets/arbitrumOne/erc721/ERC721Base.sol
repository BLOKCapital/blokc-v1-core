...
/*###############################################################################
    ...
    @notice Base contract for ERC721Facet exposing ERC721 functionality
    @dev This base contract provides ERC721 implementation adapted for diamond facets.
         It uses storage library pattern and does not require constructor or initializer.
         Name and symbol are predefined in storage.
...
/// @title ERC721Base
/// @notice Base contract providing ERC721 functionality for diamond facets
/// @dev Adapted from OpenZeppelin's ERC721 implementation to work with diamond storage pattern
abstract contract ERC721Base is IERC721, IERC721Metadata, IERC721Errors {
    ...
    function _balanceOf(address owner) internal view returns (uint256) {
        ...
        return ERC721Storage.layout().balances[owner];
    }

    function _ownerOf(uint256 tokenId) internal view returns (address) {
        address owner = ERC721Storage.layout().owners[tokenId];
        if (owner == address(0)) {
    ...
    function _name() internal pure returns (string memory) {
        return ERC721Storage.DEFAULT_NAME;
    }

    function _symbol() internal pure returns (string memory) {
        return ERC721Storage.DEFAULT_SYMBOL;
    }
    ...
    function _increaseBalance(address account, uint128 value) internal {
        unchecked {
            ERC721Storage.layout().balances[account] += value;
        }
    }
    ...
    function _update(address to, uint256 tokenId, address auth) internal returns (address) {
        ...
        if (from != address(0)) {
            ...
            unchecked {
                ERC721Storage.layout().balances[from] -= 1;
            }
        }

        if (to != address(0)) {
            unchecked {
                ERC721Storage.layout().balances[to] += 1;
            }
        }

        ERC721Storage.layout().owners[tokenId] = to;
    ...
    function _setApprovalForAll(address owner, address operator, bool approved) internal virtual {
        ...
        ERC721Storage.layout().operatorApprovals[owner][operator] = approved;
    ...
    function _supportsInterface(bytes4 interfaceId) internal view returns (bool) {
        return ERC721Storage.layout().supportsInterface[interfaceId];
    }

    function _setSupportsInterface(bytes4 interfaceId, bool supported) internal {
        ERC721Storage.layout().supportsInterface[interfaceId] = supported;
    }
}
